...
type Phase = "ride" | "explode" | "float" | "implode" | "logo";
...
function Part({ shape, w, h }: { shape: string; w: number; h: number }) {
  if (shape === "wheel")
    return (
      <div
        style={{
...
export default function HeroAnimation() {
  ...
  useEffect(() => {
    // Check if introduction has already been seen in this session
    ...
    const t4 = setTimeout(() => setPhase("logo"), 2200);
        ...
        >
          <img
            ...
            style={{
              ...
              filter: phase === "explode" ? "blur(6px) brightness(1.4)" : "none",
              // Dynamically apply drop-shadow using CSS variables instead of compiled Tailwind classes
              // @ts-expect-error style filters
              filter: `drop-shadow(0 0 60px color-mix(in oklab, var(--yellow) 30%, transparent))`,
            }}
          />
        </div>
      )}
