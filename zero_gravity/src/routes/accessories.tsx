  ...
  return (
    <div className="relative bg-background text-foreground min-h-screen">
      ...
      <section className="relative pt-36 pb-20 px-6 border-b border-border overflow-hidden">
        {/* Background Grid Lines (isolated so they do not mask the text/content opacity) */}
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <p className="mt-5 text-foreground/80 max-w-2xl text-sm md:text-base leading-relaxed">
          ...
          <div className="mt-12 flex flex-col sm:flex-row sm:items-center gap-4 max-w-3xl relative z-10">
            ...
            {/* Filter Menu Drawer with Text & 3-Dash Icon */}
            <Sheet>
              ...
              {items.map((a) => (
                <article key={a.id} className="card-dark overflow-hidden group flex flex-col">
                  <div className="relative aspect-square overflow-hidden bg-black">
                    ...
                    <span className="absolute top-3 left-3 bg-yellow text-primary-foreground text-[10px] font-display font-black tracking-widest px-2 py-1">
                      {a.cat}
                    </span>
                  </div>
                  <div className="p-5 flex flex-col gap-3 flex-1">
                    <div>
                      ...
                      {a.desc && (
                        <p className="text-foreground/85 text-xs mt-1.5 leading-relaxed line-clamp-2">
                          {a.desc}
                        </p>
                      )}
                    </div>
                    <p className="text-yellow font-display text-lg mt-auto">{a.price}</p>
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <button
                        onClick={() =>
                          add({ id: a.id, name: a.name, cat: a.cat, price: a.price, img: a.img })
                        }
                        className="text-[11px] tracking-[0.25em] uppercase font-semibold border border-yellow/60 text-yellow rounded py-2.5 hover:bg-yellow hover:text-primary-foreground transition"
                      >
                        + Add to Cart
                      </button>
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[11px] tracking-[0.25em] uppercase bg-yellow text-primary-foreground rounded py-2.5 font-bold hover:brightness-110 transition text-center"
                      >
                        Order Now
                      </a>
                    </div>
