...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const loadProductData = async () => {
      ...
      try {
        ...
        setBrandId(p.product_brand ? String(p.product_brand) : "");
        setPrice(String(p.product_price || ""));
        setQty(String(p.product_qty || "0"));
        setDescription(p.product_description || "");

        setIsActive(p.isActive === 1);
        setIsAvailable(p.isAvailable === 1);
        setIsNew(p.isNew === 1);
        setIsFeatured(p.isFeatured === 1);

        if (p.product_image) {
          setCurrentImage(p.product_image);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (directImageUrl.trim()) {
        finalImageUrl = directImageUrl.trim();
      }

      // If user uploaded a new local file to Supabase storage bucket 'products'
      if (selectedFile) {
      ...
      const { data, error: updateErr } = await updateProduct(id, updatePayload);
      if (updateErr) throw updateErr;

      setSuccessMsg(`Product "${data.product_name}" updated successfully! Redirecting...`);
      setTimeout(() => {
        navigate("/admin");
      }, 1000);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center text-white gap-3 p-8">
        <RefreshCw className="w-8 h-8 animate-spin text-[#e5a93b]" />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-zinc-800/60">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-1">
            Update specifications, reassign categories/brands, upload replacement media, and change visibility flags.
          </p>
        </div>
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                          isAvailable ? "bg-emerald-500 text-black" : "bg-zinc-800 text-zinc-500"
                        }`}
                      >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs text-zinc-400">
                    <span className="truncate max-w-[220px]">
                      {selectedFile?.name || "Stored image URL"}
                    </span>
                    <button
                      type="button"
                      onClick={handleRemoveImage}
